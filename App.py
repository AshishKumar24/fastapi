...
# 📁 Upload + SQL input
sidebar = dbc.Card([
    html.H4("Data Input", className="card-title"),
    ...
    html.Br(),
    dbc.Textarea(id="sql-base", placeholder="Base SQL query"),
    dbc.Textarea(id="sql-compare", placeholder="Compare SQL query"),
...
def run_comparison(n, join_cols, compare_cols, options, tab):
    ...
    conn.execute("DROP TABLE IF EXISTS mismatch_details")
    conn.execute("DROP TABLE IF EXISTS unique_base")
    conn.execute("DROP TABLE IF EXISTS unique_compare")
    conn.register("mismatch_details", comparison_results["mismatch_details"])
    conn.register("unique_base", base_only)
    conn.register("unique_compare", compare_only)
...
def render_tab(tab):
    ...
    if tab == "overview":
        return dbc.Row([
            ...
            ], className="kpi-card", style={"background": "linear-gradient(to right, #f97316, #f43f5e)", "color": "white"})),
            dbc.Col(dbc.Card([
